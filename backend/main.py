...
async def upload_file(file: UploadFile = File(...), project_id: str = Form(...), type: str = Form("input")):
    ...
    if type == "generation":
        ...
        result = await storage.save_upload_file(file, project_id, type=type)
...
# [新增] Bridge 本地导入接口 (接收路径，后端负责复制)
class BridgeImportRequest(BaseModel):
    project_id: str
    paths: List[str]

@app.post("/api/bridge/import")
async def bridge_import_files(req: BridgeImportRequest):
    """
    Bridge 脚本只发送路径，后端负责复制文件到 inputs 目录并广播
    """
    project_dir = settings.WORKSPACE_DIR / req.project_id
    inputs_dir = project_dir / "inputs"
    inputs_dir.mkdir(parents=True, exist_ok=True)
    
    assets = []
    for file_path_str in req.paths:
        try:
            ...
            # 2. 生成 URL
            # [Fix] 不要使用 quote 编码文件名。保持原始中文文件名，避免 rembg 等后续流程将编码后的字符串误认为是文件名。
            url = f"/files/{req.project_id}/inputs/{src_path.name}"
            ...
            logger.error(f"Failed to copy {file_path_str}: {e}")
            
    # 3. 广播通知
    if assets:
        # [修改] 不直接广播，而是提交一个 bridge_sync 任务给 dispatcher
        # 这样可以确保消息格式和处理逻辑与 WebSocket 提交的任务完全一致
        sync_task = schemas.TaskSubmit(
            task_type=schemas.TaskType.BRIDGE_SYNC,
            client_id="bridge_http_import", # 给一个独立的 client_id
            payload={"project_id": req.project_id, "assets": assets}
        )
        # 异步执行，不阻塞当前请求
        asyncio.create_task(dispatcher.dispatch(sync_task, str(uuid.uuid4()), app.state.process_pool))
