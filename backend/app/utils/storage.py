...
async def save_upload_file(file: UploadFile, project_id: str = None) -> dict:
    """保存用户上传的原图 (支持存入指定项目)"""
    ...
    save_dir = PROJECTS_DIR / project_id / "inputs"
    # URL 映射: /files/{id}/inputs/... (因为 workspace 挂载在 /files)
    url_prefix = f"/files/{project_id}/inputs"
