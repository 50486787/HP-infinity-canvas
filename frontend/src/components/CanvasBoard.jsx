...
const CanvasBoard = ({ 
    ...
    onHistoryRecord, activeWorkflow // [New] 接收 activeWorkflow
}) => {
  ...
  const handleToolbarAction = async (action) => {
    ...
    // 去底逻辑
    if (action === 'remove_bg') {
        // [Modified] 委托给上层处理 (Editor.jsx)，以便带上 project_id
        onLayerAction('rembg', selectedId);
        return;
    }
  };
  ...
  const onMouseMove = (e) => {
    ...
    // [New] 多选拖拽逻辑 (拦截 handleCanvasMouseMove)
    if (isDragging && dragInfoRef.current.initialPositions) {
        e.preventDefault();
        const { startMouse, initialPositions } = dragInfoRef.current;
        ...
        setImages(prev => prev.map(img => {
            if (initialPositions[img.id]) {
                return { ...img, x: initialPositions[img.id].x + dx, y: initialPositions[img.id].y + dy };
            }
            return img;
        }));
        return;
          ...
          if (imageId !== null) {
              ...
              if (!obj.isLocked) {
                  ...
                  if (croppingId === imageId) {
                      ...
                      const currentIds = Array.isArray(selectedId) ? selectedId : (selectedId ? [selectedId] : []);
                      // 确保当前点击的元素包含在拖拽列表中
                      const dragIds = currentIds.includes(imageId) ? currentIds : [imageId];
      ...
      <div style={{ transform: `translate(${offset.x}px, ${offset.y}px) scale(${zoom})`, transformOrigin: '0 0', position: 'absolute', top: 0, left: 0, width: '100%', height: '100%' }}>
        ...
        {images.map((img, index) => {
          ...
          >
            ...
            <ResizeHandles obj={img} selectedId={isImgSelected ? img.id : null} zoom={zoom} croppingId={croppingId} viewMode={viewMode} onMouseDown={handleMouseDown}/>
          </div>
        )})}
