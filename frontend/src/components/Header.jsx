...
import { 
  ChevronDown, ChevronUp, Undo2, Redo2, Save, FolderOpen, Package, FileJson, ArrowLeft, HelpCircle
} from 'lucide-react';
...
const Header = ({ 
  ...
  return (
    <header className={`${isCollapsed ? 'h-0 border-none' : 'h-16 border-b'} bg-white border-gray-200 transition-all duration-300 z-40 select-none shrink-0 relative`}>
      <button 
        onClick={() => setIsCollapsed(!isCollapsed)}
        className="absolute -bottom-4 left-1/2 -translate-x-1/2 bg-white border border-gray-200 rounded-b-lg px-4 py-0.5 shadow-sm hover:bg-gray-50 z-50 text-gray-400 flex items-center justify-center h-4"
        title={isCollapsed ? "展开顶部栏" : "折叠顶部栏"}
      >
        {isCollapsed ? <ChevronDown size={14} /> : <ChevronUp size={14} />}
      </button>

      <div className={`h-full w-full flex items-center justify-between px-4 ${isCollapsed ? 'hidden' : ''}`}>
