...
import { 
  ...
  Download, FileText, Image as LucideImage, Check, LayoutGrid
} from 'lucide-react';
                ...
                (img.sourceIds || []).map(sourceId => {
                    ...
                    return (
                        <path key={linkKey} d={getBezierPath(startX, startY, endX, endY)}
                            ...
                            strokeWidth={highlightedLinks.has(linkKey) ? "2" : "1.5"}
...
export const ResizeHandles = ({ obj, selectedId, zoom, croppingId, viewMode, onMouseDown }) => {
    ...
    return ( <>
        ...
        {handles.map(handle => {
           let style = { width: handleSize, height: handleSize, background: 'white', border: `${1/zoom}px solid #0096FF`, position: 'absolute', zIndex: 30 };
...
export const ContextMenu = ({ contextMenu, onLayerAction, setContextMenu, activeWorkflow, onSelect, selectedIds = [] }) => {
    ...
    else if (['image', 'shape', 'text'].includes(type)) {
      if (type === 'image') {
